    ...
    var app = new Vue({
        ...
        methods: {
            ...
            startCatalog() {
                ...
                $result = $connection->query('SELECT `top_clothes`.`ID`,`Name`,`ImageID`,`Image`,`top_clothes`.`Description`,`Cost`,`NameColor`, `NameMaterial`, `GenderID` FROM `top_clothes`, `top_clothes_image`,`color`, `material` WHERE `top_clothes_image`.`ID` = `top_clothes`.`ImageID` AND `top_clothes`.`MaterialID` = `material`.`ID` AND `top_clothes`.`ColorID` = `color`.`ID` ORDER BY `top_clothes`.`Cost` DESC;'); // запрос на выборку
                $array3 = array();
            ...
            createCookieTShirt: async function(event) {
