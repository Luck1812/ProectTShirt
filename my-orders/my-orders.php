            ...
            startOrders() {
                ...
                if ($userID != "" || null) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        $result2 = $connection->query("SELECT `sketch_image`.`ID` as `ArticleSketch`, `NumberElectronicReceipt`,`Date`,`top_clothes`.`Name`, `UserID`,`PostalCode`,`TopClothesID`,`SizeID`,`NameStatus`,`SketchImageID`,`top_clothes`.`Cost`,`StatusID`, `category_image`.`Name` as `NameSketch` FROM `electronic_receipt`,`top_clothes`, `status`, `user`, `category_image`, `sketch_image` WHERE `electronic_receipt`.`SketchImageID` = `sketch_image`.`ID` AND `sketch_image`.`CategoryImageID` = `category_image`.`ID` AND `top_clothes`.`ID` = `electronic_receipt`.`TopClothesID` AND `user`.`ID` = `electronic_receipt`.`UserID` AND `status`.`ID` = `electronic_receipt`.`StatusID` AND `UserID` = '$row[ID]';"); // запрос на выборку
                    }
                    $array3 = array();
                    $count = 1;
